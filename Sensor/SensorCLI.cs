...
public class SensorCLI
{
    ...
    public void Run(string gatewayHost, int gatewayPort = 8080, int videoPort = 8081)
    {
        ...
        try
        {
            ...
            // Iniciar heartbeat automático em background
            _sensor.StartHeartbeatAuto();
            Console.WriteLine("[OK] Heartbeat automático iniciado (a cada 5s).");
            Console.WriteLine();
    ...
    private void CmdVideo()
    {
        string framesInput = PedirInput("Número de frames a enviar (ex: 10)");
        if (!int.TryParse(framesInput, out int frameCount) || frameCount <= 0)
        {
            Console.WriteLine("[ERRO] Número de frames deve ser um inteiro positivo.");
            return;
        }

        Console.WriteLine($"A iniciar VIDEO_STREAM na porta {_videoPort}...");
        string resp = _sensor!.SendVideoStream(_videoPort, frameCount);
    ...
    private static void MostrarAjuda(List<string> tiposRegistados)
    {
        ...
        Console.WriteLine("  HEARTBEAT   — Enviar sinal de vida (manual)");
        Console.WriteLine("  VIDEO       — Enviar stream de vídeo");
        ...
        Console.WriteLine("  (Heartbeat automático ativo em background)");
